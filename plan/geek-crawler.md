...
|-----------------|---------------------|
| Query Geek-Crawler API for completed `partner` and `competitors` runs + pages at generate/preflight/tool spawn | Start partner/competitor crawls inline or store raw HTML in `content_creator_v2` |
| Extract quoteable text from `crawl_pages.Html` into WRITE prompts | Host Geek-Crawler UI, BFF, or SignalR hub in phi |
| Fail closed when required external crawl is missing | Add project-site URLs as a Geek-Crawler `crawlType` |
...
| Method | Route | Use |
|--------|-------|-----|
| GET | `/crawls/latest?crawlType=&seeds=` | Resolve latest completed run for seeds |
| GET | `/crawls/{runId}` | Run snapshot |
| GET | `/crawls/{runId}/pages` | Paginated HTML for extraction |
