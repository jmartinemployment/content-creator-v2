...
> **Restored again, 2026-09-22**, after being emptied by an unrelated frontend-cleanup commit
> (`2367b08`) that swept this doc as collateral. The Stage 2 "BLOCKED ON DATA" status and its
> `crawl_runs`/Qdrant counts below are a 2026-09-17 snapshot, not current state — `AGENTS.md`'s
> live text now describes project-site grounding as working ("Site structure... now comes from a
> Geek-Crawler-v2 project-site crawl run and is read back by Run ID"), so this gate was very
> likely cleared since. Re-verify before treating anything below as a live blocker. The heading
> semantics (h6 = tools, h5 = keyword level, h2 = message pillars) are a fact of
> `GccV2HierarchyToolMatch`, not a status, and are not documented at this level of detail anywhere
> currently live -- that part is worth trusting as-is.
