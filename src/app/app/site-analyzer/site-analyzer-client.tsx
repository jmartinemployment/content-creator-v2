...
export function SiteAnalyzerClient() {
  ...
  const [startingContentType, setStartingContentType] = useState<string>(DEFAULT_CONTENT_TYPE);
                    ...
                    <>
                      ...
                      {SHOW_GAP_GENERATE_BUTTON ? (
                        <>
                          ...
                          <label className="mt-3 block text-xs font-medium text-[var(--gcc-muted)]">
                            Content type
                            <select
                              value={startingContentType}
                              onChange={(e) => setStartingContentType(e.target.value)}
                              disabled={creating}
                              className="mt-1 block w-full rounded-md border border-[var(--gcc-line)] bg-white px-2 py-1.5 text-sm text-[var(--gcc-fg)] disabled:opacity-50"
                            >
                              {CONTENT_TYPES.map((t) => (
