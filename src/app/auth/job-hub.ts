...
/** Mirrors GeekAPI tool-source crawl hub payload (camelCase). */
export type GccV2CrawlEvent = {
  createId?: string;
  runId: string;
  status: string;
  hosts?: Array<{
...
export async function joinCrawlRun(connection: HubConnection, runId: string): Promise<void> {
  if (connection.state === HubConnectionState.Disconnected) {
    await connection.start();
  }
  await connection.invoke("JoinCrawlRun", runId);
}

export function onCrawlHubReconnected(
  connection: HubConnection,
...
export function onCrawlEvent(
  connection: HubConnection,
  handler: (evt: GccV2CrawlEvent) => void,
): () => void {
  const listener = (raw: unknown) => handler(raw as GccV2CrawlEvent);
  connection.on("CrawlEvent", listener);
  return () => connection.off("CrawlEvent", listener);
}
